...
CREATE TABLE IF NOT EXISTS `quiz_sessions` (
  ...
  `grounding_mode` VARCHAR(32) NULL COMMENT 'user_content、web_search、url_extract、mixed、private 或 hybrid',
  `user_input` LONGTEXT NOT NULL COMMENT '用户原始输入内容',
  `questions_json` JSON NOT NULL COMMENT '生成时的完整题目快照',
  `sources_json` JSON NULL COMMENT '生成时的公开来源快照，不含网页正文',
  `researched_at` DATETIME(3) NULL COMMENT '网络资料获取时间',
...
CREATE TABLE IF NOT EXISTS `knowledge_bases` (
  ...
  CONSTRAINT `fk_knowledge_bases_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户私有知识库';
...
CREATE TABLE IF NOT EXISTS `knowledge_documents` (
  ...
  CONSTRAINT `fk_knowledge_documents_kb` FOREIGN KEY (`knowledge_base_id`) REFERENCES `knowledge_bases` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE,
  CONSTRAINT `fk_knowledge_documents_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE,
  CONSTRAINT `chk_knowledge_documents_status` CHECK (`status` IN ('uploaded', 'parsing', 'chunking', 'embedding', 'ready', 'failed', 'deleting'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识库原始文档及索引状态';
...
CREATE TABLE IF NOT EXISTS `document_ingestion_tasks` (
  ...
  CONSTRAINT `fk_document_ingestion_tasks_document` FOREIGN KEY (`document_id`) REFERENCES `knowledge_documents` (`document_id`) ON UPDATE RESTRICT ON DELETE CASCADE,
  CONSTRAINT `fk_document_ingestion_tasks_kb` FOREIGN KEY (`knowledge_base_id`) REFERENCES `knowledge_bases` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE,
  CONSTRAINT `fk_document_ingestion_tasks_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE,
  CONSTRAINT `chk_document_ingestion_tasks_status` CHECK (`status` IN ('pending', 'processing', 'completed', 'failed')),
  CONSTRAINT `chk_document_ingestion_tasks_stage` CHECK (`stage` IN ('uploaded', 'parsing', 'chunking', 'embedding', 'ready', 'failed'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='文档异步索引任务';
