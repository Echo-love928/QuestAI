...
接口先把任务写入 MySQL，再以 HTTP 202 立即返回：
...
前端固定每 8 秒查询一次。任务状态为 `pending`、`processing`、
`completed` 或 `failed`；完成时返回完整 `quiz`，失败时返回安全的
业务错误码与提示。已登录用户的任务仅允许本人查询，匿名任务依赖不可预测的
128 位随机任务 ID。

原 `POST /api/v1/quiz/generate` 保留为兼容接口，新版小程序不再调用。
