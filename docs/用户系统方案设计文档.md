...
| total_xp | int | 累计经验值 |
| token_version | int | 主动统一撤销旧 Token 的版本号 |
...
|------|------|------|
| id | bigint | 主键 |
| quiz_session_id | bigint | 关联闯关会话 |
| question_id | varchar(64) | 题目业务 ID |
| question_order | int | 本次答题顺序 |
| selected_answers_json | json | 用户答案 |
| correct_answers_json | json | 正确答案快照 |
| is_correct | boolean | 是否答对 |
| duration_ms | bigint | 本题用时 |
| created_at | datetime | 创建时间 |
...
|------|------|------|
| id | bigint | 主键 |
| quiz_session_id | bigint | 关联闯关会话，唯一 |
| accuracy | decimal(5,2) | 正确率 |
| mastered_points_json | json | 已掌握知识点 |
| weak_points_json | json | 薄弱知识点 |
| three_line_summary_json | json | 三句话总结 |
| advice_json | json | 学习建议 |
| share_quote | text | 学习金句 |
| created_at | datetime | 创建时间 |

设计说明：

1. 题目保存完整 JSON 快照，答题记录按题拆分，便于历史回看和后续错题本。
2. 核心统计字段单独冗余存储，便于列表查询和聚合分析。
3. 报告的列表字段保留 JSON，兼顾结构化查询和 MVP 开发效率。
...
5. 后端生成 JWT，只包含用户 ID、Token 版本、签发/过期时间和唯一编号；openid 仅保存在服务端。
...
3. token 无效时统一返回 401；前端单飞重新登录并重试一次，失败后仅核心接口退回匿名模式。
4. 普通重新登录不会撤销旧 Token，只有安全处置时递增 `token_version` 才统一失效。
...
`POST /api/v1/quiz/tasks` 与 `GET /api/v1/quiz/tasks/{task_id}`

改造点：

1. 创建任务时增加可选用户上下文解析，并将请求快照写入
   `quiz_generation_tasks`。
2. 后台复用原有出题服务，完成后把题库写回任务记录。
3. 有登录态时，同时将题库写入 `quiz_sessions`。
4. 前端固定每 8 秒轮询；已登录任务仅允许所属用户查询。
5. 原 `POST /api/v1/quiz/generate` 保留兼容，避免影响既有调用方。
...
3. 401 时保留旧 token，通过全局单飞机制重新登录并重试一次；只有新登录完整成功后才原子替换本地 token。
4. 核心接口重新登录失败时退回匿名请求；用户接口显示可重试状态，避免死循环。
