...
import type { AnswerResult, LearningReport, Question, QuizHistoryDetail } from '@/types/api'
...
export default function HistoryDetailPage() {
  ...
  if (state === 'loading') return <View className='screen'><StatusBar /><View className='screen-body user-screen-body'><PageHeader title='闯关详情' /><View className='user-loading'><Text className='user-state-icon'>📖</Text><View className='user-state-title'>正在展开这页记录</View></View></View></View>
  if (state === 'error' || !detail) return <View className='screen'><StatusBar /><View className='screen-body user-screen-body'><PageHeader title='闯关详情' /><View className='user-error'><Text className='user-state-icon'>?</Text><View className='user-state-title'>这页记录没找到</View><View className='user-state-copy'>{message}</View>{quizId && <Button className='primary-btn user-state-action' onClick={() => load(quizId)}>重新加载 →</Button>}</View></View></View>
